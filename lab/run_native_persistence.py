#!/usr/bin/env python3
"""Evaluator for the sixth real-data cycle's memory/persistence accumulator
(PROP-NATIVE-03, lab/native_persistence.py), run on top of run5's real
BOP-LMO native-sensitivity dataset (lab/data/bop_lmo_episodes_native/).

Standalone runner, mirroring lab/run_native_sensitivity.py's own structure
...
from cqts.safety, unchanged). run5's own evaluator and results are untouched
and independently reproducible; this is a NEW file for a NEW decision
mechanism (M_k-gated ACT) layered on run5's own single-instant check.

Two variants, selected via --variant:
  a  (default) -- PROP-NATIVE-02's perturbation rule reused byte-identical;
                  only the M_k>=theta gate is added (isolates the memory
                  variable, per the handoff's stated preference).
  b            -- ALSO substitutes the residual-scaled perturbation
                  magnitude (lab/native_persistence.py:stage_decision_with_
                  variant's residual branch) for PROP-NATIVE-02's rule,
                  disclosed as a separate, materially different change (see
                  RATIONALE.md for why (a) alone was predicted, before this
                  run, to reduce to a fixed-stage rule).
Ground truth is read ONLY in `evaluate()`, after the online decision for
every stage/episode has already been made -- the same offline-oracle role
T* plays for runs 1-5.
"""
...
from lab.native_persistence import PersistenceConfig, persistent_first_act_stage
...
def evaluate(test, tasks, native_cfg: NativeConfig, pcfg: PersistenceConfig, inference, repeats):
    ...
    for ti, (name, spec) in enumerate(tasks.items()):
        ...
        for ep in test:
            idx, acted, audit = persistent_first_act_stage(ep["stages"], spec, native_cfg, pcfg)
        ...
        result["tasks"][name] = {
            ...
            "executed_native_persistence_policy": {
...
def main():
    ...
    persist_cfg_dict = cfg["native_persistence"]
    if args.variant == "a":
        pcfg = PersistenceConfig(theta=int(persist_cfg_dict["theta"]))
    else:
        pcfg = PersistenceConfig(
            theta=int(persist_cfg_dict["theta"]),
            residual_scale_ref=float(persist_cfg_dict["residual_scale_ref"]),
            residual_cap_multiplier=float(persist_cfg_dict["residual_cap_multiplier"]),
        )
    ...
    ev = evaluate(test, cfg["tasks"], native_cfg, pcfg, inference, int(cfg.get("bootstrap_repeats", 5000)))

    payload = {
        "schema_version": 2,
        "claim": (
            "native retained-sensitivity ACT/CONTINUE/HOLD stopping gated by a "
            "resetting persistence streak M_k>=theta (PROP-NATIVE-03) -- NO "
            "ground truth is used in the online decision anywhere; variant="
            + args.variant
        ),
        ...
        "protocol": {
            ...
            "decision_rule": (
                "M_k = M_{k-1}+1 if PROP-NATIVE-02's single-instant invariance "
                "check passes at stage k, else M_k = 0 (resetting streak, "
                "disclosed interpretation of the registered accumulator -- see "
                "lab/native_persistence.py docstring); ACT iff M_k >= theta"
                + ("" if args.variant == "a" else
                   "; variant b ALSO scales the perturbation magnitude by "
                   "max(1, residual_rmse_k/residual_scale_ref), capped at "
                   "residual_cap_multiplier*CAP")
            ),
            "theta": pcfg.theta,
            "lambda_ref_train_derived": native_cfg.lambda_ref,
            "cap": native_cfg.cap,
            "residual_scale_ref": pcfg.residual_scale_ref,
            "residual_cap_multiplier": pcfg.residual_cap_multiplier,
        ...
        "evidence_boundary": {
            ...
            "continuum_representation_caveat": "T_hat_k is still represented in SE(3), a continuum manifold (theory/CONTINUUM_AUDIT_20260909.md item 1); unchanged from run5",
            "mixed_units_caveat": "a single H_k eigenvector mixes 3 rotation (radians) and 3 translation (metres) components, perturbed by one shared scalar magnitude without separate nondimensionalization -- unchanged from run5",
        },
    }
