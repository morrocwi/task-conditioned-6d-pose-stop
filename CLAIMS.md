...
| The ICP normal-equations Hessian H_k=J^T J can be treated as an instance of the graph-Laplacian family L_R (PROP-DECAY-01), so that q_formal/M.07's diameter-based Fiedler floor λ₂≥4/(nD) bounds H_k's own conditioning | Refuted structurally: H_k is a fixed 6x6 SPD matrix over pose degrees of freedom with no vertex/edge structure, so a graph diameter D is undefined for it and its dimension never grows with the correspondence count n (unlike an n-node graph Laplacian). Confirmed numerically (`lab/results/real-bop-lmo-2026-09-09-run4/RESULT.md`): a real correspondence k-NN graph built separately from the same stage has n=400, D=16, giving 4/(nD)=0.000625, an unrelated number to H_k's own λ_min=0.1235 at that stage. q_formal/M.07 itself also has no Coq witness as of this run (see the paired Toledo-repo Task A attempt) |
| A closed-form decay predictor s_k,i=ρ_k^(K-k)·\|residual_i\|, ρ_k derived from H_k's ordinary matrix condition number (Kantorovich bound), produces a tighter/less-degenerate calibrated envelope than C6's fitted log-linear model on real BOP LM-O data | Refuted (`lab/results/real-bop-lmo-2026-09-09-run4/RESULT.md`): calibrated q=3.298 (~27x envelope inflation) is LARGER than both the original C6 joint q≈2.0008 (runs 1-2) and every run3 Bonferroni per-checkpoint q (2.085-2.200); certificate rate stays at exactly 0.0 for all three tasks (100% HOLD), the fourth cycle in a row to find this, with held-out coverage 95.0% (Wilson 83.5-98.6%) |
| A native retained-sensitivity ACT rule (PROP-NATIVE-01/02, H3) -- no fitted model, no calibrated completion set, no conformal quantile, no ground truth anywhere in the online decision -- perturbing T_hat_k along H_k's single smallest eigenvalue direction (magnitude proportional to 1/lambda_min, capped) and ACTing iff the task verdict is invariant, licenses ACT on real BOP LM-O data | **Yes, on 100% of test episodes for all three tasks (`lab/results/real-bop-lmo-2026-09-09-run5/RESULT.md`) -- the first of five real-data cycles to license ACT at all. But the ACT is WRONG (unsafe) on 92.5-100% of those episodes (114/120 task-episode pairs), always firing at the very first ICP stage (k=0), before any refinement. This is reported as a serious safety finding, not a success: H_k's spectral floor alone, without pairing to an absolute residual/noise-scale observable, is not a safe proxy for real pose-estimate uncertainty on this backend** |
| A memory/persistence accumulator (M_k >= theta, PROP-NATIVE-03) gating run5's unmodified single-instant native-sensitivity check recovers safety (removes/reduces the unsafe-ACT finding of PROP-NATIVE-02) on real BOP LM-O data | Refuted, in both a plain (theta-gate only) and a residual-scaled-magnitude variant (`lab/results/real-bop-lmo-2026-09-09-run6/RESULT.md`). On TRAIN and on FINAL TEST, the underlying single-instant check is near-constant across all 16 stages of every episode (no toggling to filter), so the streak counter degenerates into a fixed-delay knob: ACT fires deterministically at k=theta-1=3 on 100% of test episodes for all three tasks, unsafe on 80.0-87.5% of them (99/120 task-episode pairs) -- a modest drop from run5's 92.5-100% attributable only to 3 extra ICP iterations before commit, not to any noise-filtering property. A disclosed residual-RMSE-scaled magnitude variant produced byte-identical outcomes: the scale factor's dynamic range (~1-2x) is roughly two orders of magnitude too small relative to the gap between the perturbation cap and the real coarse-detector initial-pose error |
